//// This module has been vendored and modified from the original source available
//// at:
////
//// https://github.com/lpil/spinner/commit/cd55490d8c1698b22de8c6916999c84c8b5cc58f
////
//// That work is licensed under the Apache License, Version 2.0. A full copy of
//// that license text is made available below:
////
...
pub opaque type Spinner {
  Spinner(repeater: Repeater(State), frames: List(String))
}

type State {
  State(text: String, frames: Deque(String), colour: fn(String) -> String)
}
...
pub fn start(builder: Builder) -> Spinner {
  ...
  let init = State(builder.text, frames, builder.colour)
  let repeater = {
    use State(text:, colour:, ..), _ <- repeatedly.call(80, init, _)
    let assert Ok(#(frame, frames)) = deque.pop_front(frames)
    let frames = deque.push_back(frames, frame)
    ...
    State(text:, frames:, colour:)
  }

  Spinner(repeater, builder.frames)
}

pub fn set_text(spinner: Spinner, text: String) -> Nil {
  use state <- repeatedly.update_state(spinner.repeater)
